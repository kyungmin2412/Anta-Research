 ...
 * ?rceptNo=2024...                → "재무에 관한 사항" 표 덤프(기본)
 * ?rceptNo=2024...&section=business → "사업의 내용" 표 덤프
 */
export async function GET(request: Request) {
  ...
  const section = url.searchParams.get("section") === "business" ? "사업의 내용" : "재무에 관한 사항";

  try {
    if (rceptNo) {
      const sections = await getReportDocument(rceptNo, [section]);
      const tables = findSection(sections, section)?.tables ?? [];
