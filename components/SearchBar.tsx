...
export default function SearchBar({ autoFocus = false }: { autoFocus?: boolean }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      <div className="flex items-center gap-3 rounded-2xl bg-white px-5 shadow-card ring-1 ring-grey-100 focus-within:ring-2 focus-within:ring-blue-500">
        <svg
          viewBox="0 0 24 24"
          fill="none"
          className="h-5 w-5 shrink-0 text-grey-400"
          aria-hidden
        >
        ...
        <input
          ...
          className="h-16 w-full bg-transparent text-[17px] font-medium text-grey-900 outline-none placeholder:font-normal placeholder:text-grey-400"
          aria-label="기업 검색"
        />
        {loading && (
      ...
      {open && (results.length > 0 || error || (!loading && query.trim())) && (
        <div className="absolute inset-x-0 top-[calc(100%+8px)] z-30 overflow-hidden rounded-2xl bg-white p-2 shadow-float ring-1 ring-grey-100">
          {error ? (
            ...
            <ul className="max-h-[380px] overflow-y-auto thin-scroll">
              {results.map((item, index) => (
                <li key={item.corpCode}>
                  <button
                    type="button"
                    onMouseEnter={() => setHighlight(index)}
                    onClick={() => go(item.corpCode)}
